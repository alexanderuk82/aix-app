...
const CommandInput = React.forwardRef(({ onChange, value, ...props }, ref) => {
	return (
		<div className="flex items-center border-b px-3">
			...
			<input
				...
				value={value}
				onChange={(e) => {
					onChange(e); // Ensure onChange is called
				}}
				className="flex h-14 w-full rounded-md bg-transparent py-3 text-sm outline-none"
				{...props}
			/>
		</div>
	);
...
const CommandItem = React.forwardRef(({ className, ...props }, ref) => (
	<CommandPrimitive.Item
		ref={ref}
		className={cn(
			"relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none aria-selected:bg-slate-100 aria-selected:text-slate-900  dark:aria-selected:bg-slate-800 dark:aria-selected:text-slate-50 cursor-pointer",
			className
		)}
		{...props}
	/>
